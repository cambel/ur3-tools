...
class ParallelController(controller.Controller):
    ...
    def set_force_signal(self, target_pose):
        ...
        target_q = spalg.vector_to_quaternion(target_pose[3:])
    ...
    def set_pd_parameters(self, action):
        if len(action) == 2:
            ...
            force_kp = [get_value_from_range(act, self.base_force_kp[i], self.kpi_range, mtype=self.pd_range_type) for i, act in enumerate(action[:6])]
